...
generateFunc() 执行的时候需要动态注入数据，生成DOM树
...
解析到 `attr` 属性名后，对其通过 `addListener` 进行事件绑定，将 `tap` 与原生 `click` 之间映射， `addListener` 的事件触发的回调函数中组装了函数的event信息值，并且触发了 `sendData` 方法。
...
小程序的事件都是和 js 原生事件相互转换，小程序的tap 底层是 mouseup 事件转换来

分为几个过程：
为window绑定捕获阶段的 mouseuop事件
- 包括返回一些坐标信息、目标元素信息。
- 创建一个 exparser 事件

函数C的返回值触发目标元素的exparser事件，通过exparser.Event.dispatchEvent方法，执行这个方法就会走exparser事件系统的流程。看到这里忘了的可以回顾一下exparser组件系统章节。
...
可以看到sendData并不是直接指向逻辑层的（也就是Event这两条线）。小程序逻辑层和渲染层的通信会由 `Native` （微信客户端）做中转，逻辑层发送网络请求也经由 `Native` 转发。
...
data 和 view相互绑定，WXML中的动态数据就是data，使用setData 改变data
...
小程序的每个页面都是一个webview实例，但是逻辑层中只有一个APP实例，所有页面的逻辑都在一个逻辑线程中执行。

# 小程序路由
回顾一下小程序页面加载的原理，如果我们跳转到一个新页面，那么在渲染层中会插入一个webview，渲染层中可以有多个webview。

这样类似多页面，保留上一个页面的状态，这里对于webview的添加或删除都会有一个载体来维护，这就是路由栈。
